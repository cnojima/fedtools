...
function _extractTarball(options, done) {
  ...
  options.rootPath = path.resolve(path.dirname(options.file),
    path.basename(options.file, '.tar.gz'));
  subPath = path.basename(options.rootPath, '-package');
  options.destPath = path.join(options.rootPath, subPath);
  ...
  if (fs.existsSync(options.rootPath)) {
    ...
    tgz.extract(options.file, options.rootPath, function (err) {
      done(err, options);
    });
  }
}
