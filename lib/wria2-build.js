  ...
  async.waterfall([
      ...
      function (callback) {
        if (process.platform === 'win32') {
          log.notice('The package build will run directly on Jenkins');
          userChoices.buildLocation = TYPE_WAR_BUILD_REMOTE;
          callback();
        } else {
          if (!process.env.FEDTOOLS_PASSWORD) {
            log.notice('The package build will run directly on your machine');
...
function _displaySummaryAndConfirmation(done) {
  ...
  if (userChoices.buildLocation === TYPE_WAR_BUILD_REMOTE && userChoices.queueRequest ===
    ...
    if (process.env.FEDTOOLS_PASSWORD) {
      log.blue('Custom package id         : ', (userChoices.customWARid) ? 'yes' : 'no');
    }
    if (userChoices.buildLocation === TYPE_WAR_BUILD_REMOTE) {
      if (userChoices.queueRequest === REMOTE_ADD) {
    ...
    if (!userChoices.yuiBranch.match(/^wf2-/)) {
      defaultAnswer = false;
      defaultMessage = '[y|N]';
...
function _sendErrorEmailIfRemote(options, done) {
  ...
  if (userChoices.remote) {
    utilities.sendEmail({
      ...
      subject: 'Package remote build [ FAILED ]',
...
function _war(options, done) {
  ...
  fullWar = [
      ...
      if (userChoices.remote) {
        ...
        if (userChoices.buildLocation === TYPE_WAR_BUILD_REMOTE) {
          ...
          if (userChoices.queueRequest === REMOTE_STATUS) {
            ...
            log.info('Scheduling a package build for ' +
    ...
    function (yui3path, callback) {
      ...
      yui3Utils.copyYUItoWF2(yui3path, userChoices.tmpClonePath, function (err) {
        if (!err) {
          // need to cleanup the temporary directory if it exists
          log.info('Cleaning up temporary directory...');
          rimraf(yui3path, function () {
            // ignoring if it didnt work...
            callback();
          });
        } else {
          callback(err);
    ...
    function (sha, callback) {
      var m2Path = path.resolve(utilities.homePath(), '.m2');
    ...
    function (sha, callback) {
      var fileName, tgz, war, srcTgz, dstWar, dstTgz,
        pkgTypeTAR = true,
        htmlBodytpl, logFile, _logSuccess;

      fileName = sha + '-' + userChoices.branch + '-' + userChoices.username;
      war = fileName + '.war';
      tgz = fileName + '-package.tar.gz';
      srcWar = path.join(userChoices.tmpClonePath, 'wt2', 'wria2-documentation', 'target', war);
      srcTgz = path.join(userChoices.tmpClonePath, 'wt2', 'wria2-documentation', 'target', tgz);

      logFile = (userChoices.remote) ? path.join(userChoices.tmpClonePath, userChoices.username +
        '.log') : null;

      _logSuccess = function (type, name) {
        ...
        if (userChoices.username === 'jonathanrubstein') {
          utilities.unicorn();
        }
      };
      ...
      if (userChoices.remote) {
        ...
        dstTgz = path.join(userChoices.tmpClonePath, tgz);
        ...
        utilities.sendEmail({
          ...
          subject: 'Package remote build [ COMPLETE ]',
          attachments: logFile,
          htmlBody: mustache.render(htmlBodytpl, {
            username: userChoices.username,
    ...
    function (err, data) {
      if (err === REMOTE_EXEC_RETURN_CODE.SUCCESS) {
        if (userChoices.queueRequest === 'a') {
          log.success('The package job has been successfully scheduled');
